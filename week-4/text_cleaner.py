...
Pipeline Step: Clean and normalize extracted text for downstream ML & Entity Extraction

Features:
- Unicode NFKC normalization (ligatures, accented characters).
- Stripping unprintable ASCII/Unicode control characters.
- Intelligent whitespace and newline condensation.
- Quality validation (detection of empty, corrupt, or truncated text).
- Summary diagnostics comparing raw vs. cleaned text representations.
"""
...
def normalize_for_classification(text: str) -> str:
    ...
    norm = text.lower()
    # Replace email addresses and URLs with spaces
